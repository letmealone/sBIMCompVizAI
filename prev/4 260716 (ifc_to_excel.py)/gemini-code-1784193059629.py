...
def load_ifc(path):
    ...
    # [수정] 엑셀 집계와 동일한 판정을 사용하기 위해, 벽체(Wall)의 조각별 판정 결과도 
    # element_classification 딕셔너리에 병합하여 단일 소스로 관리함
    element_classification.update(wall_classification)
...
def get_element_hover_info(ent, wall_classification=None):
    ...
    if wall_classification:
        result, _ = wall_classification.get(ent.GlobalId, ('판정불가', ''))
        if result != '판정불가':
            lines.append(f"내/외벽 판정: {result}")
...
def get_space_wall_segment_polygon(ifc_file, wall_entity, space_entity, wall_footprint_polygon):
    ...
    if space_footprint is not None:
        trim_region = space_footprint.buffer(proximity_buffer_m) if space_footprint else None
        trimmed = []
...
def build_space_detail(ifc_file, wall_classification, space_entity):
    ...
    for e in related:
        ...
        # [수정] 벽체 전체 단위의 1차원 판정이 아닌, (Wall_GUID, Space_GUID) 조합의 조각(Segment) 단위 판정값 조회
        result, _reason = wall_classification.get((e.GlobalId, space_entity.GlobalId), ('판정불가', ''))
        
        # 하위 호환성을 위해 튜플 키가 없다면 기존 GlobalId 키 기반 검색
        if result == '판정불가' and e.GlobalId in wall_classification:
    ...
    return {
        ...
        'highlight_map': _build_highlight_map(related, equipment, wall_classification, space_entity.GlobalId),
...
def build_space_structural_breakdown(ifc_file, element_classification, space_entity):
    ...
    for e in related:
        ...
        # 1순위: 조각(Segment) 단위 판정 (Wall_GUID, Space_GUID)
        label, _reason = element_classification.get((e.GlobalId, space_entity.GlobalId), ('판정불가', ''))
        # 2순위: 전체 객체 단위 판정 폴백
        if label == '판정불가':
...
def _build_highlight_map(related, equipment, wall_classification, space_guid=None):
    ...
    for e in related:
        if e.is_a('IfcWall'):
            if space_guid:
                result, _ = wall_classification.get((e.GlobalId, space_guid), ('판정불가', ''))
                if result == '판정불가':
